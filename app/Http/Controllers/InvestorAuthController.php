<?php
    ...
    class InvestorAuthController extends Controller
    {
        ...
        public function login(Request $request)
        {
            ...
            // Use 'sanctum' guard for investors if you haven't configured a custom 'investors' guard
            // Auth::guard('investors')->attempt(...) would be for a custom guard.
            // For Sanctum with multiple auth models, token abilities are key.
        ...
        {
            // Ensure the authenticated user is an Investor model instance
            if ($request->user() instanceof Investor) {
                $request->user()->currentAccessToken()->delete();
                return response()->json(['message' => 'Successfully logged out']);
            }
            // If not an investor, it might be an UMKM user trying to logout via investor route
